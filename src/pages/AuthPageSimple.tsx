...
const AuthPageSimple: React.FC = () => {
  ...
  // This component should focus only on authentication form logic
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!isLogin) {
      if (password !== confirmPassword) {
        setError('You got that passord wrong dude.');
        return;
      }

      if (!acceptTerms) {
        setError('Oh, come on, please accept that terms, kind reminder.');
        return;
      }

      if (password.length < 6) {
        setError('Minimum 6 characters. Unless you want to get hacked and blame us later.');
        return;
      }
    }
    ...
    try {
      ...
      const user = (await Promise.race([authPromise, timeoutPromise])) as User;

      if (isLogin) {
        logger.auth.debug('Login completed, user received:', user);
        toast.success('Back for more punishment? Welcome back.');
      } else {
        logger.auth.debug('Signup completed, user received:', user);
        toast.success('Congratulations. You now exist in our database.');
      }
      ...
      if (errorMessage.includes('Authentication timeout')) {
        setError('Even our servers got bored waiting for you. Try again.');
      } else if (errorMessage.includes('Invalid login credentials')) {
        setError('Wrong credentials. Shocking, we know.');
      } else if (errorMessage.includes('User already registered')) {
        setError('That email is taken. Try being more original.');
      } else if (errorMessage.includes('Password should be at least 6 characters')) {
        setError('Your password is weaker than your WiFi signal.');
      } else if (errorMessage.includes('Unable to validate email address')) {
        setError('That email looks fake. Like your enthusiasm.');
      } else if (errorMessage.includes('Email not confirmed')) {
        setError('Check your email. Yes, including spam. We judge you.');
      } else {
        setError('Something broke. Probably not our fault.');
      }
  ...
  const getPasswordStrengthText = (strength: number) => {
    if (strength < 2) return 'Embarrassing';
    if (strength < 4) return 'Mediocre';
    return 'Barely Adequate';
  };

  // Show loading screen only if we're actually processing authentication
  // Allow users to see the form immediately while auth initializes in background
  const showLoadingSpinner = authLoading && isSubmitting;

  if (showLoadingSpinner) {
    return <SimpleLoadingSpinner message="Judging your life choices..." />;
  }
  ...
  return (
    <div className="min-h-screen w-full flex flex-col justify-center items-center relative overflow-hidden premium-hero-bg">
      ...
      {/* Background Auth Initialization Indicator */}
      {!isInitialized && (
        <div className="absolute top-4 right-4 z-30 flex items-center gap-2 bg-card/80 backdrop-blur-sm border border-border-alt rounded-lg px-3 py-2">
        ...
        <div className="premium-card p-8 bg-card/80 backdrop-blur-lg border border-border-alt shadow-xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <Button
                type="submit"
                disabled={isSubmitting || (!isLogin && !acceptTerms)}
