...
import AnimatedFlowsyAILogo from '@/components/ui/AnimatedFlowsyAILogo';
...
const NavbarNoMotion = () => {
  ...
  return (
    <nav className="border-b border-white/10 bg-gradient-to-r from-slate-900/95 via-gray-900/95 to-slate-900/95 backdrop-blur-2xl sticky top-0 z-50 shadow-2xl shadow-black/40">
      ...
      <div className="relative w-full max-w-7xl mx-auto px-4 py-3 flex items-center justify-between min-h-[70px] gap-4">
        {/* Premium FlowsyAI Logo - Fixed width container */}
        <div className="flex-shrink-0">
          <button
            type="button"
            onClick={handleLogoClick}
            className="group relative overflow-hidden bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-cyan-500/10 hover:from-blue-500/20 hover:via-purple-500/20 hover:to-cyan-500/20 border border-white/20 hover:border-white/40 rounded-2xl px-4 py-2 transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/20"
          ...
          <div className="flex items-center gap-2 bg-white/5 backdrop-blur-xl rounded-2xl px-4 py-2 border border-white/20 shadow-2xl shadow-black/20">
            {navItems.map(item => (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    'group relative overflow-hidden inline-flex h-9 items-center justify-center rounded-xl px-3 py-1.5 text-sm font-bold transition-all duration-500 ease-out focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:ring-offset-2 focus:ring-offset-transparent',
                    isActive
            ...
            {!forceUnauthenticated && isAuthenticated ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="group relative overflow-hidden bg-gradient-to-r from-emerald-500 via-teal-500 to-green-500 hover:from-emerald-400 hover:via-teal-400 hover:to-green-400 text-white font-bold rounded-xl hover:shadow-2xl hover:shadow-emerald-500/40 transition-all duration-500 ease-out border border-emerald-400/60 hover:border-emerald-300/80 inline-flex h-9 items-center justify-center whitespace-nowrap px-4 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/50 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent hover:scale-105 backdrop-blur-xl"
                >
                  {/* Animated shine effect */}
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>

                  <span className="relative z-10 flex items-center group-hover:scale-105 transition-transform duration-300">
                    <TrendingUp className="w-4 h-4 mr-2 group-hover:rotate-12 transition-transform duration-300 text-white" />
                    Buy $FlowAI
                  </span>
      ...
      {isMobileMenuOpen && (
        <div
          className="lg:hidden border-t border-white/10 bg-black/90 backdrop-blur-xl overflow-hidden animate-slide-down"
